...
class BattlePassWindow(ui.ScriptWindow):
	...
	def SetMissionInfo(self, missionId):
		...
		if info_2 >= info_3:
			self.missionInfoDict[0].SetText(localeInfo.BATTLE_PASS_MISSION_STATUS + " |cff82ff7d|H|h" + localeInfo.BATTLE_PASS_MISSION_COMPLETED + "|h|r")
		else:
			self.missionInfoDict[0].SetText(localeInfo.BATTLE_PASS_MISSION_STATUS + " |cfffeff7d|H|h" + localeInfo.BATTLE_PASS_MISSION_PROGRESS + "|h|r")
		...
		self.missionInfoDict[3].SetText(localeInfo.BATTLE_PASS_MISSION_PCT % float(info_2 * 100 / info_3))
	...
	def RefreshGeneralInfo(self):
		if self.missionList:
			...
			if self.generalInfoDict.has_key(1):
				self.generalInfoDict[1].SetText(localeInfo.BATTLE_PASS_QUEST_COUNT % missionCount)

			if self.generalInfoDict.has_key(2):
				self.generalInfoDict[2].SetText(localeInfo.BATTLE_PASS_QUEST_COMPLETE % completedMissionCount)
