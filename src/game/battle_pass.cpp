...
void CBattlePass::BattlePassRequestOpen(LPCHARACTER pkChar)
{
	...
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Quelque chose c'est mal passer. Merci d'essayer plus tard."));
		return;
	}
	...
	if (!bBattlePassId)
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Vous n'avez pas de pass de combat activer."));
		return;
	}
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Il n'y a pas de pass de combat avec cet identifiant. (%d)"), bBattlePassId);
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Il n'y a pas de pass de combat avec ce nom. (%s)"), battlePassName.c_str());
	...
	{
		if (test_server)
			pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Il n'y a pas de recompense pour ce pass de combat. (%s)"), battlePassName.c_str());
...
void CBattlePass::BattlePassRequestReward(LPCHARACTER pkChar)
{
	...
	{
		char buf[256] = { 0 };
		snprintf(buf, sizeof(buf), LC_TEXT("Le joueur %s a reussi avec succes le pass de combat %s."), pkChar->GetName(), LC_TEXT(battlePassName.c_str()));
		BroadcastNotice(buf);
...
void CBattlePass::BattlePassReward(LPCHARACTER pkChar)
{
	...
	if (!bBattlePassId)
	{
		pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Vous n'avez pas de pass de combat d'activer."));
		return;
	}
